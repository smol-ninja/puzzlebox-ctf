...
contract PuzzleBox {
    ...
    bool public isInitialized; // immutable
    address public admin; // immutable
    address payable public operator; // @dev can only be set to 0xDEAD by operator and assign yourself using operate()
    bytes32 public friendshipHash; // @dev can only be changed by admin through befriend()
    uint256 public lastDripId; // @dev can only be increased by drip()
    uint256 public dripCount; // @dev can only be increased by drip(), decreased by torch, spread, zip, creep
    uint256 public dripFee; // @dev can only be changed by initialize() and drip()
    uint256 public leakCount; // @dev can only be increased by zip() and leak()
    mapping (uint256 dripId => bool isValid) public isValidDripId; // @dev once and can only be set true by drip() and false by burnDripId()
    mapping (bytes signature => uint256 blockNumber) public signatureConsumedAt; // @dev can only be set during initialize()
    ...
    // @notice if operator calls it, the operator address is set to 0xDEAD
    modifier operation() {
    ...
    // @dev public payable function
    // can only be called once. checks isInitialized flag.
    // sets dripFee and operators address.
    function initialize(
    ...
    // @dev Register fee recipient and cut (in bps) for spread().
    // sets friendshipHash which contains list of friends and their cut.
    function befriend(
        address payable[] calldata friends,
        uint256[] calldata friendsCutBps
    )
    ...
    // @dev can only be called once by operator and then set operator to 0xDEAD
    // contract only takes eth equals to dripFee
    // increases dripCount, dripId
    // doubles dripFee
    function drip()
        external payable
        operation
    ...
    // @dev increase leakCount by 1, and transfer 1 wei to address(this)+leakCount
    function zip()
        external
        burnDripId(1)
    {
    ...
    // @dev makes sense to only call when address(this).balance is 6 wei
    function creep()
        external
        burnDripId(10)
    {
    ...
    // @dev can only be called when balance of this contract = 0, dripCount = 0, totalDripped >= 10
    // and nonce = adminSigNonce
    // hurray!
    ...
    // @dev internal function to use up a signature and update signatureConsumedAt
    ...
    // checks if dripID is valid
    // sets it to false and then reduces its valye by 1
    // emit Burned event
    ...
    // returns the hash of the friends and their cut
...
contract PuzzleBoxFactory {
        ...
        returns (PuzzleBox puzzle)
    {
        ...
        puzzle = PuzzleBox(payable(proxy));
        {
            ...
            friendsCutBps[0] = 0.015e4; // 1.5%
            friendsCutBps[1] = 0.0075e4; // 0.75%
            puzzle.initialize{value: 1337}( // sends 1337 wei
                // initialDripFee
                100, // drip Fee
                friends,
                friendsCutBps,
